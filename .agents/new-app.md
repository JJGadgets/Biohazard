...
1. `task k8s:newapp APP=<name> IMAGENAME=<img> IMAGETAG=<tag>` — the task automatically copies `kube/templates/test/` to `kube/deploy/apps/<name>/`, substituting `APPNAME`/`IMAGENAME`/`IMAGETAG`.
2. Configure the app via env vars or config file. For config files, place them in `./kube/deploy/apps/<app>/app/config/` and add a `configMapGenerator` with `disableNameSuffixHash: true` in `./kube/deploy/apps/<app>/app/config/kustomization.yaml`, then mount the config files in the containers that need them.
...
6. Remind the user to create the 1Password entries for the app's secrets and vars, matching the keys referenced in `es.yaml` and `vars.yaml`.
