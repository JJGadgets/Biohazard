...
- `envoy-external` (LB IP `${APP_IP_ENVOY_EXTERNAL}`) — for public exposure via Cloudflare DNS (`external-dns.alpha.kubernetes.io/target: ${DNS_MAIN_CF}`). Has three listeners: http, external (HTTPS, all namespaces), public (`*.jjgadgets.tech` with dedicated cert). External ClientTrafficPolicy adds stricter security headers (noindex, no-store, X-Frame-Options, nosniff, FLoC block, Referrer-Policy). BackendTrafficPolicy has 10s request timeout + 100Ki request buffer limit. External refers to apps that need to be accessed over Internet (users may not always be on VPN or VPN is too cumbersome) but is not assumed to be accessed by public (for example, Jellyfin or game servers). Public refers to Internet-facing apps that are meant to be accessed by the general public, such as a blog or a federated social media platform like GoToSocial.
